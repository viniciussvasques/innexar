...
export class AffiliateAuthController {
    ...
    async login(@Body() dto: AffiliateLoginDto) {
        ...
        if (!affiliate.password) {
            throw new UnauthorizedException('Senha não configurada. Entre em contato com o suporte.');
        }
        ...
        if (affiliate.status !== 'active') {
            if (affiliate.status === 'pending') {
                throw new UnauthorizedException('Sua conta está aguardando aprovação do administrador');
            }
    ...
    async register(@Body() dto: AffiliateRegisterDto) {
        ...
        if (existingAffiliate) {
            throw new ConflictException('Este email já está cadastrado como afiliado');
        }
