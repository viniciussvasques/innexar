...
export class AffiliatePortalController {
  constructor(
    private readonly affiliatePortalService: AffiliatePortalService,
  ) { }

  @Get("stats")
  @ApiOperation({ summary: "Obter estatísticas do dashboard do afiliado" })
  async getStats(@Req() req: AuthenticatedRequest): Promise<AffiliateDashboardStatsDto> {
    const affiliate = await this.affiliatePortalService.getAffiliateByUserId(
      req.user.id,
    );
  ...
  async getLinks(@Req() req: AuthenticatedRequest) {
    const affiliate = await this.affiliatePortalService.getAffiliateByUserId(
      req.user.id,
    );
