...
export class SystemEmailService {
  ...
  async create(data: unknown) {
    ...
    const encryptedPassword = this.encryption.encrypt(settingsData.password);
  ...
  async update(id: string, data: unknown) {
    ...
    if (settingsData.password) {
      updateData.password = this.encryption.encrypt(settingsData.password);
    } else {
  ...
  async testEmail(id: string, testEmail: string) {
    try {
      ...
      this.logger.error(`Erro ao enviar email de teste: ${err.message}`);
      return {
        success: false,
        message: err.message || "Erro ao enviar email de teste",
      };
    }
  }
}
