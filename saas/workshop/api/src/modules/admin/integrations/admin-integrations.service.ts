...
export class AdminIntegrationsService {
  ...
  async create(data: unknown) {
    return this.prisma.adminIntegration.create({ data: data as any });
  }

  async update(id: string, data: unknown) {
    await this.findOne(id);
    return this.prisma.adminIntegration.update({ where: { id }, data: data as any });
  }
  ...
  async test(id: string) {
    ...
    return {
      success: true,
      message: "Teste de integração global simulado com sucesso",
    };
  }
}
