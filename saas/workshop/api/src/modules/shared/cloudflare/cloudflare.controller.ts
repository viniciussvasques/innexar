...
export class CloudflareController {
  ...
  @ApiResponse({
    status: 200,
    description: "Status da zona retornado com sucesso",
  })
  async getZoneStatus() {
  ...
  async createDNSRecord(@Param("subdomain") subdomain: string) {
    const success =
      await this.cloudflareService.createTenantSubdomain(subdomain);
  ...
  @ApiResponse({
    status: 200,
    description: "Registro DNS removido com sucesso",
  })
  async deleteDNSRecord(@Param("subdomain") subdomain: string) {
    const success =
      await this.cloudflareService.deleteTenantSubdomain(subdomain);
  ...
  @ApiResponse({
    status: 200,
    description: "Disponibilidade do domínio verificada",
  })
