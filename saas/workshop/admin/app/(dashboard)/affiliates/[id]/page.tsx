...
export default function AffiliateDetailsPage() {
    ...
    const handleStatusChange = async (newStatus: 'active' | 'pending' | 'blocked') => {
        const statusLabels = {
            active: 'aprovar',
            pending: 'colocar como pendente',
            blocked: 'bloquear'
        };
        
        if (!confirm(`Tem certeza que deseja ${statusLabels[newStatus]} este afiliado?`)) {
            return;
        }

        try {
            await affiliatesApi.update(id as string, { status: newStatus });
            loadData();
            alert(`Afiliado ${newStatus === 'active' ? 'aprovado' : newStatus === 'blocked' ? 'bloqueado' : 'atualizado'} com sucesso!`);
        } catch (error) {
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start gap-4">
                ...
                <div className="flex items-center gap-3">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium border ${
                        ...
                        {affiliate.status === 'active' ? '✓ ATIVO' : affiliate.status === 'blocked' ? '✕ BLOQUEADO' : '⏳ PENDENTE'}
                    </span>
                    ...
                    <div className="flex items-center gap-2">
                        {affiliate.status !== 'active' && (
                            <button
                                ...
                                className="flex items-center gap-2 px-4 py-2 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 rounded-lg hover:bg-emerald-500/20 transition-all"
                            >
